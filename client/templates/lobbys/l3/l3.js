	...
	function Ball(x,y,width,height,xVel,yVel){
		...
		this.collision = function(){
			if(this.x > 330){
				this.xVel *= -1;
				p1Score+=1;
				lobbyThree.emit('score',p1Score,p2Score);
				restart();
			}
	...
	function actionPerformed(){
		if(on){
			...
			*/
			...
			lobbyThree.on('score',function(p1,p2){
				p1Score = p1;
				p2Score = p2;
			});

			lobbyThree.on('game',function(bool){
				game = bool;
			});

			lobbyThree.on('p1V',function(p1Vel,p1Y){
				p1.yVel = p1Vel;
				if(p1.y != p1Y){
					p1.y = p1Y;
				}
			...
			lobbyThree.on('p2V',function(p2Vel,p2Y){
				p2.yVel = p2Vel;
				if(p2.y != p2Y){
					p2.y = p2Y;
				}
	...
	function p1Collsion(){
		if(ball.x < p1.x){
			if(ball.y >= p1.y && ball.y <= p1.height+p1.y){
				...
				ball.xVel *= -1;
				lobbyThree.emit('ball',ball.xVel,ball.yVel,ball.x,ball.y);
				lobbyThree.on('ball',function(bxVel,byVel,bx,by){
	...
	document.onkeydown = function(event){
		switch(event.keyCode){
			...
			case 87:
				...
				p1.yVel = -4;
				lobbyThree.emit('p1V',p1.yVel,p1.y);
			break;
			// s key
			case 83:
				p1Down = true;
			...
			case 38:
				...
				p2.yVel = -4;
				lobbyThree.emit('p2V',p2.yVel,p2.y);
			break;
			// down arrow
			case 40:
				p2Down = true;
